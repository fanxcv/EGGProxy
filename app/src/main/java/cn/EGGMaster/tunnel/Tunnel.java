...
public abstract class Tunnel {
    ...
    private final static ByteBuffer CON_BUFFER = ByteBuffer.allocate(2048);
    private final static ByteBuffer GL_BUFFER = ByteBuffer.allocate(8192);
    ...
    protected ByteBuffer m_SendRemainBuffer;
    ...
    public void onConnectable() {
        try {
            if (m_InnerChannel.finishConnect()) {//连接成功
                onConnected(CON_BUFFER);//通知子类TCP已连接，子类可以根据协议实现握手等。
            } else {//连接失败
                this.dispose();
            }
        } catch (Exception e) {
